 ...
 * Function:	Type::size
 *
 * Description:	Return the size of the Type.
 *      Rules:
 *          - array of T: sizeof(T) * length
...
{
    if(_kind == FUNCTION || _kind == ERROR)
        return 0;
    if (_kind == ARRAY)
    {
        if (_indirection > 0)
            return 4 * _length;
        return (_specifier == CHAR ? 1 : 4) * _length;
    }
    return _specifier == CHAR ? 1 : 4;
}
