...
#include <assert.h>
/****************************************************************
 * Modifications for COEN 177 lab.								*
 * 1. Alter the priority queue such that there is a chance of 	*
 * 	  selecting a lower priority job.							*
 ***************************************************************/
#include "debug.h"
#include "kernel.h"
#include "proc.h"
...
not_runnable_pick_new:
	 ...
	 * process. If there is still nothing runnable we "schedul// 1.e" IDLE again
	 */
 ...
 */
  ...
  for (q=0; q < NR_SCHED_QUEUES; q++) {	
	/* 1. 2% chance of picking a random queue */
	if ((q > 3) && (rand() % 100 > 97)){
